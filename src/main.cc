...
// The engine will need to call genValidMoves on all pieces, so we could
...
// The downside to this is it is very slow. The way I am optimizing it right now for the playable version is
// that it only checks if you are in chec4457a7ck on the specific move you call. For all other valid moves, it won't check.
